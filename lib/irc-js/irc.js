...
var net = require( 'net' )
  ...
  , DEBUG
  , DEBUG_RECIEVED_MESSAGES 

/* ------------------------------ CONSTANTS ------------------------------ */
DEBUG = false
DEBUG_RECIEVED_MESSAGES = false
...
function IRC( options ) {
  ...
  function parseMessage( data ) { var buffer, last, line, message, command, i
    ...
    try {
      for ( i = 0; i < buffer.length; i++ ) {
        ...
        if ( ( command = message.command.toLowerCase()) !== 'error' ) {
          emitter.emit( command, message )
          if (command === "001") {
            internal.nick = message.params[0]
            console.log("Set internal nick to " + internal.nick);
          }
          if (command === "nick" && message.person.nick === internal.nick) {
            internal.nick = message.params[0]
            console.log("Set internal nick to " + internal.nick);
          }
        }
